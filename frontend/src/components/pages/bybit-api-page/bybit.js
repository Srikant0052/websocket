...
function Bybit() {
  ...
  useEffect(() => {
    ...
    //Emit events
    socket.emit("bybitPair", data);

    //Listen for events
    socket.on("bybitPair", async function (data) {
      //   console.log(data);
      let show = await data;
      setSocketData(show);
    });
  ...
  async function saveBybitData() {
    try {
      let response = await axios({
        method: "post",
        url: "http://localhost:4000/bybitApi",
      });
            ...
            {socketData.length > 0 ? (
              socketData.map((coin, index) => (
                <tr key={index}>
                  ...
                  <td>{coin.volume}</td>
                  <td>{coin.bidPrice}</td>
                  <td>{coin.priceChangePercent.toFixed(4)}</td>
                </tr>
              ))
            ) : (
              <tr>
