...
function Indoex() {
  ...
  useEffect(() => {
    ...
    //Emit events
    socket.emit("indoexPair", data);

    //Listen for events
    socket.on("indoexPair", async function (data) {
      //   console.log(data);
      let show = await data;
      setSocketData(show);
    });
  ...
  async function saveIndoexData() {
    try {
      let response = await axios({
        method: "post",
        url: "http://localhost:4000/indoexApi",
      });
            ...
            {socketData.length > 0 ? (
              socketData.map((coin, index) => (
                <tr key={index}>
                  <td>{index + 1}</td>
                  <td>{coin.pairName}</td>
                  <td>{coin.lastPrice.toFixed(15)}</td>
                  <td>{coin.lowPrice.toFixed(15)}</td>
                  <td>{coin.highPrice.toFixed(15)}</td>
                  <td>{coin.baseVolume.toFixed(15)}</td>
